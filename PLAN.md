...
**Last updated:** 2026-09-14 (v1 roadmap's 39 requirements all shipped since 2026-09-06, but substantial v1.x feature work has landed since without PLAN.md being kept current — see "Post-v1 Feature Additions" below for the catch-up. Multi-tenancy foundation migrations/code are complete but not yet confirmed live — see that section.)
...
### Post-v1 Feature Additions (undocumented until this 2026-09-14 catch-up)

Between the last real PLAN.md narration (2026-09-06, third Phase 8 session) and now, ~65 commits shipped real product surface that was never recorded here. None of this maps to the original 39 requirements — it's v1.x scope the user asked for directly during build sessions. Summarized from `git log` + reading the current components, not re-derived requirement-by-requirement:

- **Gastos (condo expense tracking) — new admin feature, full CRUD.** `condo_expense_categories`/templates/instances schema (`supabase/migrations/20260909020000_gastos_schema.sql` + a soft-delete follow-up), fixed and variable expense templates with per-installment editable amounts, a daily cron (`app/api/cron/*`) that auto-generates fixed-expense instances, an admin list/filter screen (`/gastos`), a create form (`/gastos/new`), a templates screen (`/gastos/plantillas`), and a paid-history screen (`/gastos-pagados`). Feeds a "pending expenses this month" dashboard KPI (replacing the original saldo-a-favor KPI tile). **Not yet added to the Requirements table above or given a REQ-ID** — do that (or explicitly mark it out-of-scope-for-numbering) next time this file gets a real pass.
- **Daily USD/Bs exchange-rate reference** (BCV + Binance sources), admin-editable, surfaced via `components/dashboard/ExchangeRateCard.tsx` and folded into the dashboard KPI grid. Payments in Bs/USDT now show a computed $ reference amount. Payment currency was also decoupled from cuota currency (a cuota can be denominated in one currency, paid in another, converted at the recorded rate) — **this is a meaningful scope change from RPRT-03/PMNT-04's original "never convert across currencies" framing** and should get a real decision note, not just a commit message, next time this file is touched.
- **Resident payment self-reporting**, previously out-of-scope (V5/NOTF-* were explicitly deferred in Phase 7's writeup). Residents can now upload a payment screenshot from `ReportPaymentDialog.tsx` (`Mi hogar`), OCR-prefills the form fields from the receipt image, and it lands in an admin review queue (`/pagos-reportados`, `components/paymentReports/PaymentReportsPageClient.tsx`) with confirm/reject actions that auto-register the payment on confirm. Every confirm/reject and every gasto-marked-paid action now writes to `condo_audit_logs` (previously unused — PLAN.md's original scope explicitly listed "audit log browsing UI" as out-of-scope for v1, but writing to the log for these two flows apparently wasn't).
- **Resident portal shell rebuilt.** Phase 7's bottom tab bar (`ResidentTabBar.tsx`) was replaced by `components/resident/ResidentSidebar.tsx` — a desktop-sidebar/mobile-hamburger shell deliberately mirroring `components/admin/AdminSidebar.tsx`'s structure (per the user's own request, per the code's comments), not the mockup's mobile-app pattern. House info was split out of Mi hogar into its own **Perfil page** (`components/resident/PerfilClient.tsx`) — **this directly contradicts Phase 7's "V6 Mi perfil intentionally NOT built" note above**, which is now stale and should be corrected there, not just here. A new `CommunityBalanceCard.tsx` shows a community-wide (not just per-house) balance on Mi hogar.
- **Visual/theme pass:** switched from Once UI's default theme to a light theme with brand-blue solid buttons (`c3374e4`); dashboard KPI cards redesigned as a quiet-outline `StatCard` component in a real 4-column grid (replacing an earlier broken/uneven layout); income charts restyled.
- **Multi-tenancy groundwork** (below) also landed in this window, including a fix making resident login carry `community_id` in its session cookie.

**None of this has been reconciled against the Requirements table, the Phase 7/8 write-ups above, or given real "Decisions made" entries** — the entries above are inferred from commit messages and a light read of the current code, not confirmed with the user. Treat this section as a flag for a proper documentation pass, not as authoritative as the phase sections above (which were written contemporaneously with the work).
...
**If applying via `psql`/the Supabase CLI rather than the Dashboard SQL Editor:** wrap each file in `BEGIN; ... COMMIT;` — the Dashboard editor already runs each paste as one transaction, but a bare `psql -f` does not, and a mid-file failure (e.g. an unexpected NULL surfacing despite this branch's fixes) would otherwise leave that one migration partially applied.

**Only one community exists in production today (ASOBARCELONA).** Everything above is additive/backward-compatible for admin behavior — nothing about how the admin app works changes. Two things ARE user-visible:
- **Every resident gets signed out once.** The resident session cookie's shape changes (it now carries `community_id`); an existing cookie fails the new validation and the resident simply has to log back in with house + PIN — no data loss, just one extra login.
- **Apply all 7 migrations to the live Supabase project BEFORE deploying this branch's code.** `lib/actions/residentAuth.ts`'s `residentLogin` now expects `condo_verify_house_pin` to return `community_id` (migration 7); if the code ships first, resident login would break until the migrations catch up (a runtime guard was added so this fails as a clear error rather than a silent broken cookie — see the code, but don't rely on it as a substitute for the right deploy order).
...
**Two more onboarding blockers for a second community, found in final review, not fixed here (documentation only — same "don't onboard tenant 2 yet without addressing this" category as the resident-login gap above):**
- **Exchange rates:** `lib/actions/exchangeRate.ts`'s `setExchangeRate` (admin-triggered) and `app/api/cron/exchange-rate/route.ts` (the scheduled cron) both insert into `condo_exchange_rates` without setting `community_id` explicitly. The autofill trigger falls back to `condo_default_community_id()` ("the oldest community") for both — so the admin-triggered path would get its insert silently REJECTED by RLS for any community other than the oldest one (the trigger stamps the wrong tenant, then `with check` rejects it), and the cron path (service-role, bypasses RLS) would silently keep writing every community's rate onto the oldest community's row forever. Needs real per-community handling before a second community goes live.
- **Expense categories:** `condo_expense_categories` is now tenant-scoped, but the only rows that exist are the 5 seeded once for ASOBARCELONA (`supabase/migrations/20260909020000_gastos_schema.sql`) and there is no in-app path to create more. A second community would see an empty category list on `gastos/new` and be unable to create any expense template until categories are seeded for it directly in the database.
